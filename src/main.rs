use git2::{Cred, RemoteCallbacks, BranchType, Repository, ObjectType, Signature, IndexAddOption, PushOptions};
use std::path::Path;
use std::env;
use dotenv::dotenv;
use clap::{App, Arg};
...
fn clone_repo(url: &str, path: &Path, pat: &str, username: &str) -> Result<(), git2::Error> {
...
fn search_and_replace_in_file(file_path: &std::path::Path, search: &Regex, replace_with: &str) -> io::Result<()> {
...
fn get_repo_info() -> Result<(String, String, String, String, String), Box<dyn std::error::Error>> {
...
fn create_checkout_and_update_branch(
    repo: &Repository, 
    branch_name: &str,
    path_string: &str, 
    exclude_dir: &Regex, 
...
fn commit_changes(repo: &Repository, message: &str) -> Result<(), git2::Error> {
...
fn push_to_remote(repo: &Repository, pat: &str, branch_name: &str) -> Result<(), git2::Error> {
